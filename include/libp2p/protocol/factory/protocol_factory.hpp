  ...
  class ProtocolFactory {
   public:
    ...
    struct ProtocolConfig {
      ...
      bool enable_relay =
          true;  // Requires: autonat (and transitively identify)
      bool enable_holepunch_server =
          true;  // Requires: relay (and transitively autonat, identify)
      bool enable_holepunch_client =
          false;  // Independent - usually only enabled for clients

      // Optional callbacks for protocol completion events
      std::function<void()>
          identify_callback;                   // Called when Identify completes
      ...
      std::function<void()>
          holepunch_server_callback;  // Called when Holepunch Server completes
      std::function<void()>
          holepunch_client_callback;  // Called when Holepunch Client completes

      /**
       * @brief Auto-enable required dependencies to create a valid
       * configuration
     ...
     * @param config - protocol configuration specifying which protocols to
     * create
     ...
     * @throws InvalidProtocolConfigException if configuration has invalid
     * dependencies
     */
    template <typename Injector>
    static ProtocolSet createProtocols(std::shared_ptr<Host> host,
...
namespace libp2p::protocol::factory {
  ...
  inline void ProtocolFactory::validateConfig(const ProtocolConfig &config) {
    ...
    if (config.enable_holepunch_server && !config.enable_relay) {
      throw InvalidProtocolConfigException(
          ...
          "Holepunch Server uses relay capabilities for establishing direct "
          "connections.");
    }

    // Note: Holepunch Client can work independently as it's typically used by
    // clients that don't need to provide relay services themselves
  }

  template <typename Injector>
  ProtocolFactory::ProtocolSet ProtocolFactory::createProtocols(
      std::shared_ptr<Host> host, const ProtocolConfig &config,
      const Injector &injector) {
    ...
    if (config.enable_identify) {
      protocols.identify =
          createIdentify(host, injector, config.identify_callback);
    }

    if (config.enable_autonat) {
      protocols.autonat =
          createAutonat(host, injector, config.autonat_callback);
    }
    ...
    auto msg_processor = std::make_shared<IdentifyMessageProcessor>(
        ...
        *injector.template create<std::shared_ptr<peer::IdentityManager>>(),
        injector.template create<
            std::shared_ptr<crypto::marshaller::KeyMarshaller>>());
  ...
  std::shared_ptr<HolepunchServer> ProtocolFactory::createHolepunchServer(
    ...
    return std::make_shared<HolepunchServer>(*host, msg_processor,
                                             host->getBus());
  }
  ...
  std::shared_ptr<HolepunchClient> ProtocolFactory::createHolepunchClient(
    ...
    return std::make_shared<HolepunchClient>(*host, msg_processor,
                                             host->getBus());
  }
