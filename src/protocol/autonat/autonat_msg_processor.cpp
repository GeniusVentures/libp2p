...
namespace libp2p::protocol {
  ...
  void AutonatMessageProcessor::autonatParseDIALREQUEST(
      const StreamSPtr &stream, autonat::pb::Message &msg,
      std::string &peer_id_str, std::string &peer_addr_str) {
    return;
    ...
    auto peer_id_res = peer::PeerId::fromBytes(gsl::span<const uint8_t>(
        reinterpret_cast<const uint8_t *>(peer_info.id().data()),
        peer_info.id().size()));
    ...
    // We must avoid participating in a DDOS by filtering addresses that do not
    // match the address we are connected to.
    ...
    for (const auto &addr : peer_info.addrs()) {
      ...
      if (ma_addr.value().getStringAddress().find(remote_ip)
          != std::string::npos) {
    ...
    if (!matching_addresses.empty()) {
      ...
      for (const auto &addr : matching_addresses) {
        // Consider whether duplicate addresses could cause us to report twice a
        ...
        temphost->newStream(
            ...
            [self{shared_from_this()}, stream, addr](auto &&stream_res) {
              ...
              auto rw =
                  std::make_shared<basic::ProtobufMessageReadWriter>(stream);
              if (!stream_res) {
                ...
                rw->write<autonat::pb::Message>(
                    responsemsg, [self, stream](auto &&res) mutable {
                      self->log_->info(
                          "Sent a negative DIAL_RESPONSE to autonat request");
                    });
                return;
              }
              ...
              std::cout << "Successfully created new stream to target."
                        << std::endl;
            });
      }
      ...
      log_->error("Peer has no matching addresses for AUTONAT dial. {}",
                  peer_id.toBase58());
      stream->close([self{shared_from_this()}, p = std::move(peer_id_str),
                     a = std::move(peer_addr_str)](auto &&res) {
        if (!res) {
  ...
  void AutonatMessageProcessor::autonatParseDIALRESPONSE(
      const StreamSPtr &stream, autonat::pb::Message &msg,
      std::string &peer_id_str, std::string &peer_addr_str) {
    ...
    if (local_addr_res.has_error()) {
      log_->error("DIAL_RESPONSE missing local address from stream. {}",
                  local_addr_res.error().message());
      return;
    }
    if (!msg.dialresponse().has_status()) {
      log_->error("DIAL_RESPONSE missing status. {}",
                  msg.dialresponse().statustext());
    ...
    if (response_ma.has_error()) {
      log_->error("DIAL_RESPONSE bad address. {}",
                  response_ma.error().message());
      return;
    }

    if (msg.dialresponse().status() == autonat::pb::Message::E_DIAL_ERROR) {
      log_->info("Address {} has a dial error, this will tally up.",
                 response_ma.value().getStringAddress());
      unsuccessful_addresses_[std::string(
          response_ma.value().getStringAddress())]++;
    } else if (msg.dialresponse().status() == autonat::pb::Message::OK) {
      log_->info("Address {} has an OK status, this will tally up.",
                 response_ma.value().getStringAddress());
      successful_addresses_[std::string(
          response_ma.value().getStringAddress())]++;
    } else {
      log_->info(
          "Autonat DIAL_RESPONSE has had an error that does not indicate NAT "
          "status: {}",
          msg.dialresponse().statustext());
      // signal_autonat_received_(false);
    }
    // Record confirmation
    if (successful_addresses_[std::string(
            response_ma.value().getStringAddress())]
        >= 4) {
    ...
    if (unsuccessful_addresses_[std::string(
            response_ma.value().getStringAddress())]
        >= 4) {
