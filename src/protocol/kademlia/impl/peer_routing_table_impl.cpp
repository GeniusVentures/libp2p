...
boost::optional<PeerId> Bucket::removeReplaceableItem() {
  ...
  for (auto it = peers_.rbegin(); it != peers_.rend(); ++it) {
      // Replace "and" with "&&" and "not" with "!"
      if (it->is_replaceable && !it->is_connected) {
    ...
    outcome::result<bool> replacePeer(Bucket& bucket,
      ...
      event::Bus& bus) {
        ...
        if (!removed.has_value()) {
          return outcome::failure(PeerRoutingTableImpl::Error::PEER_REJECTED_NO_CAPACITY);
        }
