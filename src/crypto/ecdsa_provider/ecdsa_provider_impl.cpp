...
namespace libp2p::crypto::ecdsa {
  ...
  outcome::result<PublicKey> EcdsaProviderImpl::derive(
    ...
    OUTCOME_TRY((auto &&, ec_key), convertBytesToEcKey(key, d2i_ECPrivateKey));
  ...
  outcome::result<Signature> EcdsaProviderImpl::signPrehashed(
    ...
    OUTCOME_TRY((auto &&, ec_key), convertBytesToEcKey(key, d2i_ECPrivateKey));
    OUTCOME_TRY((auto &&, signature), GenerateEcSignature(message, ec_key));
  ...
  outcome::result<bool> EcdsaProviderImpl::verifyPrehashed(
    ...
    OUTCOME_TRY((auto &&, ec_key), convertBytesToEcKey(public_key, d2i_EC_PUBKEY));
    OUTCOME_TRY((auto &&, signature_status),
